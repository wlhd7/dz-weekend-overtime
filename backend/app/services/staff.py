...
class StaffService(BaseService):
    ...
    def get_staffs_by_department(self, department_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result: List[Dict[str, Any]] = [dict(staff._mapping) for staff in staffs]
            ...
            self._log_error(
                "get_staffs_by_department", e, {"department_id": department_id}
            )
    ...
    def get_sub_departments(self, department_id: int) -> List[SubDepartment]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(sub_depts)} sub-departments for department {department_id}"
            )
            return sub_depts
            ...
            raise HTTPException(
                status_code=500, detail="Failed to retrieve sub-departments"
            )

    def add_staff(
        self, name: str, department_id: int, sub_department_id: Optional[int] = None
    ) -> bool:
        ...
        try:
            ...
            if sub_department_id:
                self._validate_sub_department_belongs_to_department(
            ...
            existing_staff = (
                self.db.query(Staff)
                .filter(Staff.name == name, Staff.department_id == department_id)
                .first()
            )
            ...
            if staff_in_other_dept:
                ...
                self.db.flush()
                self._ensure_overtime_week(cast(int, new_staff.id))
                success = self._commit_or_rollback(
                    "staff_added",
                    {
                        "staff_name": name,
    ...
    def remove_staff(self, name: str, department_id: int) -> bool:
        ...
        try:
            ...
            if not staff:
                self._log_error(
                    "remove_staff_not_found",
                    ValueError("Staff not found"),
                    {"name": name, "department_id": department_id},
                )
                raise HTTPException(
            ...
            self._log_error(
                "remove_staff", e, {"name": name, "department_id": department_id}
            )
            raise HTTPException(status_code=500, detail="Failed to remove staff")

    def _validate_sub_department_belongs_to_department(
        self, sub_dept_id: int, dept_id: int
    ) -> None:
        """Validate that sub-department belongs to the specified department."""
        self._validate_id(sub_dept_id, "Sub-department ID")

        sub_dept = (
            self.db.query(SubDepartment).filter(SubDepartment.id == sub_dept_id).first()
        )
        ...
        dept_id_value = cast(Any, getattr(sub_dept, "department_id"))
        if dept_id_value != dept_id:
            raise HTTPException(
                status_code=400,
                detail="Sub-department does not belong to the specified department",
            )

    def _ensure_overtime_week(self, staff_id: int) -> None:
        """Ensure an overtime week record exists for staff."""
        existing = (
        ...
        if not existing:
            self.db.add(OvertimeWeek(staff_id=staff_id))
