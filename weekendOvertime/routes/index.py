...
def index():
    ...
    # fetch staffs by department_id FK,
    # include sub-department name when available
    staffs = db.execute(
        """
        SELECT
            staffs.*,
        ...
        ON staffs.sub_department_id = sub_departments.id
        LEFT JOIN sat
        ON sat.staff_id = staffs.id
        LEFT JOIN sun
        ON sun.staff_id = staffs.id
