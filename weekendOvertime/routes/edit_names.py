        ...
        try:
            if sub_id is not None:
                cur = db.execute(
                    ...
                    (name, department_id, sub_department_id)
                    VALUES (?, ?, ?)',
                    """(
                        name, dept_id, sub_id
                    )
            ...
            # If insert was ignored (existing name),
            # update department and sub-department when needed
            if cur.rowcount == 0:
                existing = db.execute(
                    """
                    SELECT department_id, sub_department_id FROM staffs
                    WHERE name = ?', (name,)
                    """
                ).fetchone()
                needs_update = False
                ...
                if needs_update:
                    try:
                        ...
                        db.execute(
                            f"""
                                UPDATE staffs SET {', '.join(update_fields)}
                                WHERE name = ?
                            """,
                            tuple(update_params),
                        )
